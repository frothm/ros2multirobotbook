...
The Dispatcher then compares all the `BidProposals` received and submits a `rmf_task_msgs/DispatchCommand` message with the fleet name of the robot that the bid is awarded to. There are a couple different ways the Dispatcher evaluates the proposals such as fastest to finish, lowest cost, etc which can be configured.
